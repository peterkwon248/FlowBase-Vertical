...
프론트는 한 겹 더 있다 — **동결 목업 + [§21](docs/§21-표면-근거-재배치.md) + [§22](docs/§22-커버리지-부재의-일급-취급.md)**, 충돌 시 §가 목업을 이긴다.
...
| [docs/ADR-021-묶음-무엇을-합쳐-볼-것인가.md](docs/ADR-021-묶음-무엇을-합쳐-볼-것인가.md) | **다음 손.** 어느 파일들을 합쳐 대시보드로 볼 것인가 — 수동 묶음 + 「전체」. 조사(Linear·Lightroom·Gmail)·버린 접근·안 정한 것 4개 |
