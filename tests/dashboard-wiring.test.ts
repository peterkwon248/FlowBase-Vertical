...
run("대시보드 1단 — 화면 숫자 = CLI 숫자", () => {
  ...
  it("비용 구성이 실제 항목을 담는다 — 클레임이 사라지지 않는다", async () => {
     ...
     *
     * 광고비는 **사용자가 그 배치를 되돌리면** 0이 되므로 등장 여부를 데이터가
     * 정하게 두고 있었다. 그런데 바로 위 줄은 「매출원가는 안 나온다」를 **절대값으로**
     * 박아 뒀다 — 근거가 「원가는 기준 데이터가 아직 없어서 0이다」였다.
     *
     * **그 전제가 2026-08-16에 만료됐다.** 사용자가 원가 71건을 넣자 매출원가가
     * 34,811,200원이 되어 카드에 등장했고 이 단언이 빨개졌다. 회귀가 아니라
     * **제품이 한 걸음 나아간 것**이다. 같은 규칙을 테스트하는 두 줄 중 하나만
     * 데이터에 물어보고 있었으므로, 아래와 같은 모양으로 맞춘다.
     */
    if (s.pnl.cogs > 0) expect(labels, "원가가 있는데 안 그렸다").toContain("매출원가")
    else expect(labels, "원가 0을 그리면 없는 비용을 있는 것처럼 말한다").not.toContain("매출원가")
    /**
     * ★ 이 줄은 **잠들어 있던 동안 낡았다** (2026-08-20 감사 B-2) ★
     *
     * 원래는 `adDirect + adUnallocated > 0 → 라벨 「광고비」`였다. 두 군데가 틀렸고,
     * 둘 다 **이 시험이 2026-08-15 이후 한 번도 안 돌아서** 아무도 몰랐다:
     *
     *   ① 라벨이 「직접 광고비」로 바뀌었다        (`dashboard.ts` · 08-17)
     *   ② 미배분을 더하면 **층을 섞는 것**이다     (같은 커밋)
     *
     * 미배분 광고비는 이 그림에서 **빠지는 게 맞다** — 누락이 아니라 층이다.
     * 이 카드는 「매출을 나눈 것」이고 항등식이 `일곱 항 + 기여이익 = 매출`인데,
     * 미배분은 기여이익 **아래** 칸(채널 기여이익)에서 빠진다. 더하면 합이 매출을
     * 넘고, 도넛은 그 합을 원으로 정규화하므로 **즉시 거짓말이 된다.**
     *
     * ⚠ **오늘 `adDirect`는 늘 0이다** — `snapshot.ts`의 리터럴(감사 0순위 결함).
     * 그래서 아래 `else` 가지만 돌고 있다. 이 시험의 초록을 「광고비 배선 확인됨」으로
     * 읽으면 안 된다. 그 결함이 고쳐지는 날 `if` 가지가 자동으로 켜진다.
     */
    if (s.pnl.adDirect > 0) {
      expect(labels, "직접 광고비가 있는데 안 그렸다").toContain("직접 광고비")
    } else {
      expect(labels, "광고비 0을 그리면 없는 비용을 있는 것처럼 말한다").not.toContain("직접 광고비")
    }
    /**
     * ★ 층이 섞이지 않았다 — 이 단언이 새로 생긴 것이다 ★
     * 08-17에 미배분을 이 목록에서 뺐는데 **그걸 지키는 시험이 없었다.** 누가 다시
     * 더해도 아무 데서도 안 걸린다. 오늘 미배분이 15,700,534원이라 실효가 있다.
     */
    expect(
      labels,
      "미배분 광고비가 이 카드에 올라왔다 — 합이 매출을 넘고 도넛이 거짓말한다",
    ).not.toContain("광고비")
  })
