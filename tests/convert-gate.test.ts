...
const DEVIATIONS: { field: IrField; from: string[]; to: string[]; why: string }[] = [
  ...
  /**
   * ★ 「가져오기 기록」 장부의 머리글 — 「연결」이 「출처」가 됐다 (2026-08-21 · ADR-028) ★
   *
   * 장부의 단위가 배치에서 **파일**로 올라가자(015) 그 칸이 답하는 질문이 바뀌었다.
   * Fact는 연결에서 오지만 **기준 데이터에는 연결이 없다** — 015는 거기에 상수
   * 「채널 없음」을 넣었고, 그 구분은 바로 옆 「유형」 칸이 이미 하고 있었다.
   * 같은 말을 두 칸이 하는 동안 **어느 파일인지는 화면 어디에도 없었다** —
   * 원가 파일 둘을 넣으면 두 줄이 구별되지 않는다 (남은일 3.11).
   *
   * **열을 신설하지 않았다.** 11열이 되면 목업 레이아웃이 바뀌고 §21-3 재배치
   * 대상이 하나 늘어난다. 바뀌는 것은 **글자 두 개**이고 구조·클래스·토큰은
   * 그대로다 — 015가 「유형」 칸에 한 것과 같은 종류의 수선이라 E-1의 「발명」이
   * 아니다 (§21-8).
   */
  {
    field: "texts",
    ...
    to: ["시각", "출처", "유형"],
    why:
      "가져오기 기록 장부의 머리글 — 「연결」 → 「출처」 (ADR-028 · §21-8). 기준 " +
      "데이터에는 연결이 없어 그 칸이 상수 「채널 없음」이었다. 이제 Fact는 연결, " +
      "기준·훑기만은 **파일 이름**을 싣는다. 열 수·순서·스타일은 그대로다",
  },
  {
    field: "texts",
...
const S21_REGIONS = [
  ...
  },
  {
    id: "ad-rule",
    removeWhenWired: null,
    mockupStyle: "height: 26px; padding: 0 6px; font-size: 11px; min-width: 132px",
    why:
      "★ 광고비 표의 「배분 규칙」 — 목업의 `<select>`를 **사실 한 줄로 바꿨다** " +
      "(2026-08-21 · `data-s21=\"ad-rule\"`). ADR-022가 「배분 규칙을 발명하지 않는다」로 " +
      "닫았기 때문이다: 배분은 파일의 「광고집행 옵션ID」가 이미 말하고 있고, 안분이나 " +
      "전환매출 비율로 나누면 **비용의 임자가 바뀐다**(A 광고를 보고 B를 샀다고 B가 A의 " +
      "광고비를 지는 게 아니다). 고를 것이 없는 select는 «못 누르는데 그려진 것»이라 " +
      "U-3 위반이고, 그 자리는 대신 **그 캠페인의 돈이 어디 있고 무엇을 하면 상품에 " +
      "붙는지**를 말한다 — ADR-022가 미배분을 세 갈래로 센 이유가 그것이다",
  },
] as const
