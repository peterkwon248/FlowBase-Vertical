...
describe("§22-3 3절 문장", () => {
   ...
   *
   * 그리고 같은 일이 ESM에서 한 번 더 일어났다 (2026-08-14, 파워클릭 프로파일).
   * `["order"]` → `["ad", "order"]`. 예고된 반복이라는 것이 요점이다 — 프로파일을
   * 더하는 것 말고는 아무 파일도 손대지 않았는데 채널 화면 문장이 함께 움직였다.
   *
   * 9칸(마켓 3 × 성격 3) 중 읽을 수 있는 칸이 **3 → 5**가 됐다.
   */
  it("팩이 읽을 수 있는 성격을 프로파일에서 파생시킨다 — 손으로 적지 않는다", () => {
    ...
    expect(sorted("esm"), "파워클릭 프로파일이 들어오며 ad가 붙었다").toEqual(["ad", "order"])
