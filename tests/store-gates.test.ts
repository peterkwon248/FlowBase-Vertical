...
async function seed(db: Driver): Promise<void> {
  await migrate(db)
  await db.prepare(`INSERT INTO library (id, name, created_at) VALUES (?,?,?)`).run(
    LIB,
    "기본",
...
describe("합격 기준 1 — 적재 → 되돌리기 → 행수 원복", () => {
  ...
  it("청크 적재 후 되돌리면 행이 사라지고 배치가 undone이 된다", async () => {
    const b = batch("b-1")
    await repo.openBatch(b)
    await repo.loadChunk("fact_order", b, [order(1), order(2), order(3)])
    await repo.commitBatch("b-1", "2026-08-12T00:01:00Z")

    const inRange = (): Promise<number> =>
      repo.countInRange("active_order", LIB, "ordered_at", "2026-07-01", "2026-07-31")

    expect(await inRange()).toBe(3)
    expect(await repo.sumInRange("active_order", "total_amount", LIB, "ordered_at", "2026-07-01", "2026-07-31")).toBe(6000)

    const removed = await repo.undoBatch("b-1", "2026-08-12T00:02:00Z")
    expect(removed).toBe(3)
    expect(await inRange()).toBe(0)
    expect((await repo.batchStatus("b-1"))?.status).toBe("undone")
  })

  it("여러 배치 중 하나만 되돌린다", async () => {
    const b1 = batch("b-1")
    const b2 = batch("b-2")
    await repo.openBatch(b1)
    await repo.loadChunk("fact_order", b1, [order(1), order(2)])
    await repo.commitBatch("b-1", "t")
    await repo.openBatch(b2)
    await repo.loadChunk("fact_order", b2, [order(3), order(4)])
    await repo.commitBatch("b-2", "t")

    expect(await repo.countInRange("active_order", LIB, "ordered_at", "2026-07-01", "2026-07-31")).toBe(4)
    await repo.undoBatch("b-1", "t")
    expect(await repo.countInRange("active_order", LIB, "ordered_at", "2026-07-01", "2026-07-31")).toBe(2)
  })

  it("적재 중(open)인 배치는 활성 조회에 나타나지 않는다", async () => {
    const b = batch("b-1")
    await repo.openBatch(b)
    await repo.loadChunk("fact_order", b, [order(1)])
    // 아직 커밋 전 — 사실이 아니다.
    expect(await repo.countInRange("active_order", LIB, "ordered_at", "2026-07-01", "2026-07-31")).toBe(0)
    await repo.commitBatch("b-1", "t")
    expect(await repo.countInRange("active_order", LIB, "ordered_at", "2026-07-01", "2026-07-31")).toBe(1)
  })

  it("같은 source_key 재가져오기는 UPSERT다 (헌장 B-2)", async () => {
    const b1 = batch("b-1")
    await repo.openBatch(b1)
    const first = await repo.loadChunk("fact_order", b1, [order(1), order(2)])
    expect(first).toEqual({ inserted: 2, updated: 0 })
    await repo.commitBatch("b-1", "t")

    const b2 = batch("b-2")
    await repo.openBatch(b2)
    const second = await repo.loadChunk("fact_order", b2, [
    ...
    // 행이 늘지 않고 값만 갱신됐다.
    expect(await repo.countInRange("active_order", LIB, "ordered_at", "2026-07-01", "2026-07-31")).toBe(3)
    const row = await db.prepare(`SELECT total_amount, version, batch_id FROM fact_order WHERE source_key='SK-1'`).get()
  ...
  it("제외 행이 사유와 함께 남고 되돌리기로 함께 사라진다", async () => {
    ...
    expect((await repo.batchStatus("b-1"))?.excluded_count).toBe(2)
    await repo.commitBatch("b-1", "t")
    await repo.undoBatch("b-1", "t")
...
describe("합격 기준 3 — 되돌리기 후 재가져오기 시 조정 생존 (헌장 B-3)", () => {
  ...
  it("조정은 배치를 되돌려도 살아남고 재가져오기 후에도 그대로다", async () => {
    const b1 = batch("b-1")
    await repo.openBatch(b1)
    await repo.loadChunk("fact_order", b1, [order(1)])
    await repo.commitBatch("b-1", "t")
    ...
    const b2 = batch("b-2")
    await repo.openBatch(b2)
    await repo.loadChunk("fact_order", b2, [order(1)])
    await repo.commitBatch("b-2", "t")

    // 원본은 돌아왔고 조정도 그대로다 — 유효값 = 원본 + 조정 스택.
    expect(await repo.countInRange("active_order", LIB, "ordered_at", "2026-07-01", "2026-07-31")).toBe(1)
    const adj = await repo.adjustmentsFor(CONN, "fact_order", "SK-1")
    expect(adj).toHaveLength(1)
    expect(adj[0]?.new_value).toBe("1200")
    expect(adj[0]?.reason).toBe("정산서 기준 정정")
  })

  it("조정 스택은 여러 겹이 시간순으로 쌓인다", async () => {
    const b = batch("b-1")
    await repo.openBatch(b)
    await repo.loadChunk("fact_order", b, [order(1)])
    await repo.commitBatch("b-1", "t")
    ...
    const stack = await repo.adjustmentsFor(CONN, "fact_order", "SK-1")
    expect(stack.map((a) => a.new_value)).toEqual(["1200", "1300", "1250"])
  })

  it("대사 확인은 조정과 별개다 — 조정이 불일치를 지우지 못한다", async () => {
    ...
    const ack = await db.prepare(`SELECT difference FROM recon_ack WHERE recon_key = ?`).get("2026-07/settlement")
    expect(ack?.difference).toBe(-12400)
  })
})

describe("합격 기준 4 — 활성 로드 (전체 테이블 적재 경로 부재)", () => {
  it("대량 적재 후에도 조회는 범위 결과만 돌려준다", async () => {
    ...
    expect(await repo.countInRange("active_order", LIB, "ordered_at", "2026-07-01", "2026-07-31")).toBe(2500)
    // 집계도 SQL이 한다.
    expect(
      await repo.sumInRange("active_order", "total_amount", LIB, "ordered_at", "2026-07-01", "2026-07-31"),
    ).toBe(250_000)
    await db.close()
  })
})
