...
export function coverageNote(cc: ConnectionCoverage, dict: MarketDict | null): string {
  ...
  //      말하지 않는다 ★ 커버리지의 `cogs`는 열린다(입력이 존재하게 되므로) —
  //      그런데 **손익의 매입원가는 안 움직인다.** `fact_order_item`이 비어 있어
  //      원가를 곱할 대상이 없기 때문이다. 여기서 «열립니다»라고 하면 §22-3-a가
  //      금지한 «못 지킬 약속»이 된다. 그 사실은 «온전함»의 몫이라 `pnlGaps`의
  //      `cogs-unappliable`과 상품 화면 게이지가 말한다 (§22-3-b 관할 분리).
  //
