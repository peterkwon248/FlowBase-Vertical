...
/**
 * ★ 「지금」은 **KST**다 (ADR-009 ④ · 2026-08-20 수정) ★
 *
 * ─────────────────────────────────────────────────────────────
 * ADR-009는 「기간 경계는 **KST(UTC+9) 자정**」이라 정했고 파서(`parseDateLike`)는
 * 그 규율을 지킨다. 그런데 **「지금」을 만드는 자리 다섯이 전부 UTC였다.**
 *
 * 그 결과 사용자가 겪던 것:
 *   · 매월 1일 **아침 00~09시(KST)**에 앱을 열면 **지난달** 손익이 뜬다
 *   · 그 시간대에 저장한 원가는 `effective_from`이 **전날**로 박힌다
 *     (원가는 적용일이 키라, 하루 어긋나면 그 하루의 손익이 통째로 다른 값이 된다)
 *
 * ★ 왜 `getFullYear()`류를 안 쓰나 ★
 * 그건 **실행 기기의 시간대**를 읽는다. 노트북을 들고 여행하면 답이 달라진다 —
 * ADR-002가 파서에서 UTC 게터를 강제한 이유와 같은 이유다. 여기서는 epoch에
 * **+9시간을 더하고 UTC 게터로 읽는다.** 기기가 어디에 있든 답이 같다.
 *
 * ★ `nowStamp`는 여기 안 낀다 — 아래 그 함수의 주석 참조 ★
 * ─────────────────────────────────────────────────────────────
 */
const KST_OFFSET_MS = 9 * 60 * 60 * 1000

/** KST 벽시계를 ISO 문자열로. 자르는 길이는 부르는 쪽이 정한다. */
const kstNow = (): string => new Date(Date.now() + KST_OFFSET_MS).toISOString()

/**
 * 오늘이 속한 달(KST). 데이터가 하나도 없을 때의 기간이다 — 그때 화면은 어차피 빈다.
 *
 * `App.tsx`가 초기 상태 두 곳에서 이 값을 쓴다. 전에는 그쪽이 같은 식을 **복제**하고
 * 있었고, 그래서 고칠 자리가 셋이었다.
 */
export const thisMonth = (): Month => kstNow().slice(0, 7) as Month
...
/**
 * 오늘(`YYYY-MM-DD`, **KST**). 원가 입력의 기본 적용일이자 «지금 유효한 원가»의 기준일이다.
 *
 * 업무 날짜라 KST다 — 사용자가 아침 8시에 넣은 원가가 「어제부터」로 저장되면 안 된다.
 * 위 `kstNow` 주석 참조.
 */
export const today = (): string => kstNow().slice(0, 10)
 ...
 *
 * ★ 여기는 **UTC를 유지한다** — 위 `today`/`thisMonth`와 다르다 (2026-08-20 판단) ★
 *
 * 업무 날짜(기간 경계·적용일)와 **감사 시각**은 성격이 다르다. 그리고 이 값은
 * 혼자 쓰이지 않는다 — 같은 DB에 **트리거가 `datetime('now')`로 쓰는 시각**이 있다
 * (`002-row-shadow.sql`의 `row_shadow.created_at` 등, 마이그레이션 전체에서 21곳).
 * SQLite의 `datetime('now')`는 **UTC**다.
 *
 * 그래서 이쪽만 KST로 바꾸면 **한 DB 안에 시계가 둘이 되고 9시간 어긋난다** —
 * 되돌리기 순서(ADR-004)가 그 두 값을 나란히 놓는 날 조용히 뒤집힌다.
 * 「지금」을 고치는 김에 여기까지 손대는 것이 자연스러워 보이지만, 그건 트리거
 * 21곳을 함께 바꾸는 마이그레이션이고 **그건 이 수정의 범위가 아니다.**
 *
 * 즉 규칙은 이렇게 갈린다:
 *   **사람이 읽는 날짜 = KST** (달·적용일)  ·  **기계가 정렬하는 시각 = UTC** (여기)
 */
