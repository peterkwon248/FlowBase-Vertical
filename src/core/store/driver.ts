...
export const FILE_PRAGMAS: readonly string[] = [
  ...
  /**
   * ★ 이 줄은 **WAL과 한 묶음이다** — 세션 목록에서 여기로 옮겼다 (2026-08-16) ★
   *
   * 원래 근거가 "WAL에서 NORMAL은 프로세스 크래시에 안전하고 OS 크래시에서만
   * 마지막 트랜잭션을 잃는다"였는데, **그 문장은 WAL을 전제한다.** 롤백 저널
   * 모드에서 NORMAL은 저널이 동기화되기 전에 본체를 고칠 수 있어 전원이 끊기면
   * **파일이 깨질 수 있다** — 잃는 것의 크기가 다르다.
   *
   * 앱은 롤백 모드로 돈다(위 참조). 그래서 앱에는 이 완화를 걸지 않고 SQLite
   * 기본값(FULL)으로 둔다. 대량 적재가 느려지는 대가는 있지만 지금까지도 그
   * 상태였고(앱이 PRAGMA를 아예 안 걸었다), 속도는 나중에 살 수 있어도
   * 깨진 파일은 못 산다.
   */
...
export const SESSION_PRAGMAS: readonly string[] = [
  /**
   * ★ 경합에 즉시 죽지 않는다 (2026-08-16 추가) ★
   *
   * 기본값은 **0** — 다른 연결이 잠그고 있으면 기다리지 않고 그 자리에서
   * `database is locked`다. 하네스가 그것으로 한 번 빨개졌고(세 워커가 같은 파일),
   * 앱도 같은 노출을 갖고 있었다: 사용자가 앱을 켜 둔 채 CLI를 돌리면 그렇다.
   *
   * 5초인 이유는 «사람이 기다릴 수 있는 시간»과 «영영 멈춘 것처럼 보이는 시간»의
   * 사이라서다. 넘으면 여전히 실패하고, 그 실패는 이제 화면이 말한다(`take`).
   */
