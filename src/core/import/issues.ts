...
export const ISSUE_KINDS = {
  ...
  /**
   * ★ 안 받은 컬럼에 돈이 실렸다 (2026-08-21 · 조사 2.13) ★
   *
   * `missing_column`의 **거울상**이다 — 저쪽은 「선언한 컬럼이 파일에 없다」이고
   * 이쪽은 **「파일에 값이 있는데 우리가 안 받았다」**이다. 둘 다 파일 사건이다.
   *
   * 왜 필요했나: 11번가 정산 59컬럼 중 8개만 매핑한다. 나머지는 `unmappedColumnCount`
   * 라는 **수 하나**로 뭉개지고, 그 수는 0원인지 100만 원인지 구분하지 않는다.
   * 도서산간배송비가 0인 달에는 아무 일도 아니지만 **0이 아닌 달에는 그 금액이
   * 조용히 사라진다** — LOCK 6이 금지하는 그것이다.
   *
   * ⚠ **미매핑 숫자 컬럼을 전부 감시하지 않는다.** 그러면 `NO`·`수량`·`판매가`처럼
   * **이미 집계에 든** 컬럼이 매 파일마다 울린다. 상시 오탐은 사람이 오류 목록을
   * 안 보게 만든다(같은 프로파일의 VAT note가 이미 그 판단을 했다). 무엇을 감시할지는
   * **프로파일이 지목한다**(`unmappedColumns.watch`) — 마켓 지식이라 팩에 산다 (LOCK 4).
   */
  unmapped_money: {
    fatal: false,
    scope: "file",
